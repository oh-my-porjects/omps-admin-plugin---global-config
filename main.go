...
// 后台 worker 正确写法：
//
//	func (p *GlobalConfigPlugin) Init(ctx PluginContext) error {
...
//	}
//
//	func (p *GlobalConfigPlugin) runTicker() {
...
var Routes = map[string]http.HandlerFunc{
	...
	"GET /{admin_prefix}/api/global_config/config/list":           handleConfigList,
	"GET /{admin_prefix}/api/global_config/config/detail":         handleConfigDetail,
	"POST /{admin_prefix}/api/global_config/config/update":        handleConfigUpdate,
	"POST /{admin_prefix}/api/global_config/config/reset-default": handleConfigResetDefault,
	// 后台管理接口示例（以 /{admin_prefix}/api/ 开头，部署时替换为项目 UUID）
	"POST /{admin_prefix}/api/global_config/admin/ping": handleAdminPing,
...
func handleConfigList(w http.ResponseWriter, r *http.Request) {
	Plugin.handleConfigList(w, r)
}

func handleConfigDetail(w http.ResponseWriter, r *http.Request) {
	Plugin.handleConfigDetail(w, r)
}

func handleConfigUpdate(w http.ResponseWriter, r *http.Request) {
	Plugin.handleConfigUpdate(w, r)
}

func handleConfigResetDefault(w http.ResponseWriter, r *http.Request) {
	Plugin.handleConfigResetDefault(w, r)
}

// GlobalConfigPlugin 实现 GamePlugin 接口
//
// 接口定义：
//
...
type GlobalConfigPlugin struct {
	db             *sql.DB
	logger         *slog.Logger
	lifecycleCtx   context.Context
...
func (p *GlobalConfigPlugin) Name() string    { return "global-config" }
func (p *GlobalConfigPlugin) Version() string { return version }
...
func (p *GlobalConfigPlugin) Shutdown(ctx context.Context) error {
	fmt.Printf("[%s] 插件关闭\n", p.Name())
...
func (p *GlobalConfigPlugin) handleHello(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, 0, map[string]any{"module": p.Name(), "version": p.Version()}, "ok")
}

func (p *GlobalConfigPlugin) handleAdminPing(w http.ResponseWriter, r *http.Request) {
