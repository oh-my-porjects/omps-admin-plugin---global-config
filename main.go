...
var Routes = map[string]http.HandlerFunc{
	// 前台接口示例（以 /api/ 开头）
	"GET /api/global-config/hello": handleHello,
	// 后台管理接口示例（以 /{admin_prefix}/api/ 开头，部署时替换为项目 UUID）
	"POST /{admin_prefix}/api/global-config/admin/ping": handleAdminPing,
...
func (p *TemplatePlugin) Name() string    { return "global-config" }
